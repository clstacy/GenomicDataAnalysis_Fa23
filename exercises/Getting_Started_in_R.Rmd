...
R is a useful tool for analyzing data. Let's download a data file from GitHub to work with.
...
Let's pull some data files from a current project in our lab. We will use a file that has been modified for us to practice using RStudio.
...
library(readr)
msn2_4_mutants_EtOH <- read_delim("https://raw.githubusercontent.com/clstacy/GenomicDataAnalysis_Fa23/main/data/ethanol_stress/msn2-4_mutants_EtOH.txt", 
    delim = "\t", escape_double = FALSE, 
    locale = locale(encoding = "ISO-8859-1"),
    trim_ws = TRUE)
View(msn2_4_mutants_EtOH)
...
Do remember that this function uses the package readr (a part of the tidyverse package we loaded above). If you don't have that package (1) installed and (2) loaded into your script, it won't work.
...
We see in the output there are `r nrow(DE_results_heatshock_annotated)` rows and `r ncol(DE_results_heatshock_annotated)` columns in the data. The same information should be available in the environment panel of RStudio
