...
-   Utilize the limma package to identify differentially expressed proteins
    \_\_\_\_\_\_\_.
...
res_all_proteome <- treat(fit_all_proteome,
                 fc = 1.2,
                 robust = TRUE)

# save top results to an object
top.table_proteome <- topTable(res_all_proteome, sort.by = "p", n = Inf)
...
```{r}
summary(decideTests(res_all_proteome))
```

You can change the `lfc` parameter in the `treat()` code above, and see how this results summary changes.
...
data.frame(go_results_proteome) %>% View()
...
```{r enrich-KEGG-protein, eval=FALSE}
...
# get top results
top.table_transcriptome <- topTreat(res_all_transcriptome, sort.by = "p", n = Inf)
...
```{r}
# get list of genes DE at each time point, as well as direction of DE
...
Look at correlations of genes that were **upregulated** at the mRNA level.
```{r}
res_all_proteome %>%
  as.matrix() %>%
  as.data.frame() %>%
