...
This last visualization takes a while to run, so you don't need to run it. It allows you to create a heatmap of the peak locations within the promoters.
...
ChIP peaks in R: Let's generate a gene list corresponding to peaks that appear in t20 minutes but not t0 minutes. We already have the peaks annotated to genes, so we just need to get a 
