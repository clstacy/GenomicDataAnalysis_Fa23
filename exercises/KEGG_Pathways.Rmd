...
topKEGG(k, sort="up")
```
Notice I chose to sort on p-value for the "upregulated" genes (aka higher in the mutant). We could change "up" to "down" in the code above to sort on the other column.


## Pathway Visualization

So, we've found out which KEGG pathways are enriched in our gene list for this comparison. A common thing we will want to do is visualize those enriched pathways.  The Bioconductor package `pathview` allows us to visualize these pathways.
```{r}
...
# save the logFC values from res object to a new variable
...
In addition to just the genes which are included in the list of signficant genes for the KEGG pathway, we might also want to see the magnitude of fold change those 
