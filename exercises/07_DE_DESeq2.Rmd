...
Question 4: We analyzed differential expression of the counts generated by the full Salmon counts. Load in the counts generated by Salmon and look at the same contrast we did in class. What differences and similarities do you see?
...
# simplify the column names to make them pretty
colnames(dds_salmon) <- str_split_fixed(colnames(dds_salmon), "\\.", n = 2)[, 1]

# filter low counts
keep_salmon <- rowSums(counts(dds_salmon) >= 10) >= smallestGroupSize
dds_salmon <- dds_salmon[keep_salmon,]
...
summary(res_salmon, alpha=0.05)

# visualize results
DESeq2::plotMA(res_salmon, alpha=0.05)
```
