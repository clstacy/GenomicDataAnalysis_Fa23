...
dir_bam_files <- path.expand("~/Desktop/Genomic_Data_Analysis/Data/Bam/ChIP/testing_curl_files")
...
bam_dir=~/Desktop/Genomic_Data_Analysis/Data/Bam/ChIP/testing_curl_files/
...
MSN24_ESR_t20vst0_peaks <- read.delim("~/Desktop/Genomic_Data_Analysis/Analysis/ChIP/MSN24_ESR_t20vst0_peaks.xls", header = TRUE, comment.char = "#") |>
  ...
  dplyr::filter(X.log10.pvalue. > 5) |>
  # filter on magnitude of enrichment
  dplyr::filter(fold_enrichment > 4)

jeffs_MSN24_ESR_t20vst0_peaks <- read.delim("~/Downloads/MSN24_ESR_t20vst0_peaks.xls", header = TRUE, comment.char = "#")

carsons_MSN24_ESR_t20vst0_peaks <- read.delim("~/Desktop/Genomic_Data_Analysis/Analysis/ChIP/MSN24_ESR_t20vst0_peaks.xls", header = TRUE, comment.char = "#")


# see if using his changes it.
MSN24_ESR_t20vst0_peaks <- jeffs_MSN24_ESR_t20vst0_peaks |>
  # filter on significance of peaks
  dplyr::filter(X.log10.pvalue. > 5) |>
  # filter on magnitude of enrichment
  dplyr::filter(fold_enrichment > 4)
...
MSN24_ESR_t20vst0_peaks_GR <-
  GenomicRanges::GRanges(
    ...
    IRanges::IRanges(MSN24_ESR_t20vst0_peaks[, "start"], MSN24_ESR_t20vst0_peaks[, "end"])
  )
```
...
streme_msn24_ChIP <- runStreme(peak_seqs,
        control="shuffle",
        minw = 4, # default is 8, for yeast I use 5
        maxw= 8, #default is 15, for yeast I use 20
        parse_genomic_coord=FALSE,
        thresh=0.2,
        patience=5,
        silent=FALSE,
        outdir = path.expand(paste0(out_dir, "streme_msn24_ChIP_diauxic2"))
        )
...
streme_tomtom_msn24_ChIP <-
  runTomTom(
    ...
    outdir = path.expand(paste0(out_dir, "tomtom_streme_msn24_ChIP_diauxic2"))
  )
```
