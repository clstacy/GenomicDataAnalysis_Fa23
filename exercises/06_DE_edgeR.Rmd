...
-   Utilize the edgeR package (@edgeR) to identify differentially expressed
    genes.
...
Question 5: We analyzed the counts generated by Salmon using the full
fastq files. Load in the counts generated by Salmon with the subset
fastq files and look at DE genes for the same contrast we did in class.
What differences and similarities do you see? What do you think is
causing this difference?

## A template set of code chunks for doing this is below:

We already loaded in the subset counts as the object `counts_subset`
above. If you don't have that file, this code chunk will download it
from GitHub.

```{r load-salmon}
path_github_subset_counts <- 'https://github.com/clstacy/GenomicDataAnalysis_Fa23/raw/main/data/ethanol_stress/counts/salmon.gene_counts.merged.yeast.tsv'

counts_subset <- read_tsv(path_github_subset_counts, col_names = TRUE) %>%
  column_to_rownames("Name")
```
...
y_subset <- DGEList(counts_subset, group=group)
colnames(y_subset) <- sample_metadata$Sample

# add gene names
y_subset$genes <- AnnotationDbi::select(org.Sc.sgd.db,keys=rownames(y_subset),
                                        columns="GENENAME")

# filter low counts
keep_subset <- rowSums(cpm(y_subset) > 60) >= 4
y_subset <- y_subset[keep_subset,]

# calculate norm factors
y_subset <-  calcNormFactors(y_subset)

# estimate dispersion
y_subset <- estimateDisp(y_subset, design, robust=TRUE)

# generate the fit
fit_subset <- glmQLFit(y_subset, design, robust=TRUE)

# test our contrast of interest
res_subset <- glmQLFTest(fit_subset, contrast = my.contrasts[,"EtOHvsWT.MSN24ddvsWT"])

# generate a beautiful table for the pdf/html file.
topTags(res_subset, n=Inf) %>% 
  data.frame() %>% 
  arrange(FDR) %>%
  mutate(logFC=round(logFC,2)) %>%
  mutate(across(where(is.numeric), signif, 3)) %>%
...
is.de_subset <- decideTestsDGE(res_subset, p.value=0.05)
summary(is.de_subset)

# visualize results
plotSmear(res_subset, de.tags=rownames(res_subset)[is.de_subset!=0])
```
