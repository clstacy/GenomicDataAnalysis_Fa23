...
KEGG_data <- kegg_results %>%
  ggkegg(
      ...
      convert_first = FALSE,
      convert_collapse = "\n",
    pathway_number = pathway_number, # the 24th entry in kegg_results is glycolysis, changing this to change the pathway.
            convert_org = c("sce"),
            delete_zero_degree = TRUE,
            return_igraph = FALSE)

WT.glycolysis.graph <- KEGG_data$data |>
  filter(type == "gene") |>
  mutate(showname=strsplit(name, " ") |> str_remove_all("sce:")) |> 
  mutate(showname = gsub('c\\(|\\)|"|"|,', '', showname)) |>
  separate_rows(showname, sep = " ") |>
...
msn24.glycolysis.graph <- glycolysis_KEGG$data |>
  filter(type == "gene") |>
  mutate(showname=strsplit(name, " ") |> str_remove_all("sce:")) |> 
  mutate(showname = gsub('c\\(|\\)|"|"|,', '', showname)) |>
  separate_rows(showname, sep = " ") |>
...
WT.glycolysis.graph + msn24.glycolysis.graph +
