...
p_load("edgeR", "AnnotationDbi", "org.Sc.sgd.db", "pathview",
       "clusterProfiler", "KEGGgraph")# for dotplot
```
...
KEGG_reactions <- keggList("reaction") |> as.data.frame() |>
  rownames_to_column("reactionID") |>
  mutate(reaction = str_split_i(`keggList("reaction")`, ";", 1)) |>
  select(reactionID, reaction)
...
tmp |>
  ...
  left_join(KEGG_reactions, by=c(""=""))
  ggplot(aes(x=x,y=y)) +
  ggrepel::geom_label_repel(aes(label = showname, fill = logFC), box.padding = 0.0,direction = "y") +
