...
for (contrast in names(contrasts)) {
  ...
  mutate(logFC=round(log2FoldChange,2)) %>%
  mutate(across(where(is.numeric), signif, 3)) %>%
  print(shiny::tagList(reactable()))
}


res_list$EtOHvsWT.MSN24ddvsWT %>%
  data.frame() %>% 
  mutate(across(where(is.numeric), signif, 3)) %>%
  arrange(padj) %>% rownames_to_column("gene_id") %>% View()
  reactable()
```

```{r}
is.de <- subset(res_list$EtOHvsWT.MSN24ddvsWT, padj<0.01, abs(log2FoldChange) > 1)
summary(is.de)
```
Let's take a quick look at the differential expression
```{r}
plotMA(res_list$EtOHvsWT.MSN24ddvsWT)
```
