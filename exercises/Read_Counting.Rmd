...
reference_annotation = "~/Desktop/reference_genome/Saccharomyces_cerevisiae.R64-1-1.110.gtf.gz"
```
...
correct encoding is specified during alignment. If unsure, one can examine the first
several quality strings in the FASTQ file. A good rule of thumb is to check whether
lower-case letters are present (+64 encoding) or absent (+33).
...
```{r save-fc}
saveRDS(object = fc, file = "~/Desktop/yeast_fc_output.Rds")
```
...
qs <- qualityScores(filename="~/Desktop/filtered/filtered_subset_phenol_heat_rep4_rfastp_R1.fastq.gz",
                    nreads=50)
...
# Salmon pseudomapping & counting

```{bash}
ANALYSIS_DIR="/Users/$USER/Desktop/Genomic_Data_Analysis/Analysis"
...
1. Identify which gene has the highest counts across all samples.
