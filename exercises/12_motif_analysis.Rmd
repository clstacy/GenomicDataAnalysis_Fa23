...
if (!command -v $HOME/meme/bin/meme &> /dev/null) {
...
Now we have the 500bp upstream sequences for (almost) all of the genes in the
genome. Note that for yeast, this isn't too much data for a laptop.
If you're working with a larger genome, you might need to subset the
gene list down just to the genes in your gene list.
...
Let's get that list of genes with large magnitude and statistically significant negative logFC values now:
...
This was the first algorithm developed in the MEME-suite, and is still widely used. However, it is not always the best approach. Unlike many bioinformatic software, the default parameter settings are usually **not** the best options for motif analysis. 
We can adjust these settings in class to parameters better suited to our data (based on biological domain knowledge). See all of the parameter options at: https://meme-suite.org/meme/doc/meme.html
...
## Run Streme enrichment

A newer alternative function, part of the MEME suite, that is better
suited to looking for shorter motifs which are common in Eukaryotes.

For `runStreme`, properly setting the `control` parameter is key to discovering biologically relevant motifs. Often, using `control = "shuffle"` will produce a suboptimal set of motifs; however, some discriminative analysis designs don't have proper "control" regions other than to shuffle.

For our analysis, we can use the promoter sequences from the entire genome as a background
to model the null distribution, let's create an object in R with all of the 500bp upstream sequences from `seq` to use as a control
...
motifs in databases. The TomTom algorithm, part of the MEME-suite, allows us to just that, comparing our identified motifs to known motifs in annotation databases.

First, we need to download the YEASTRACT database file called (YEASTRACT_20130918.meme). We will pull this file from Github, but you
...
skn7_NaCl_down <- FC_list |>
  filter(logFC..WT.v.skn7.mutant..NaCl.response < -1.5) |>
  filter(FDR..WT.v.skn7.mutant.NaCl.response<0.01) |>
  dplyr::select(ID) %>%
  # add the upstream seqs as a new column
  left_join(seq, by=c("ID" = "ensembl_gene_id")) |>
  drop_na("coding_gene_flank")
